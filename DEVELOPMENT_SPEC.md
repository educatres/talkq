...
- `moderator.html`：講師以管理密鑰取得權限、審核問題、開關提問、匯出 CSV 或刪除留言板。
...
```text
talks/{talkId}
  talk_id, title, default_publish_mode, questions_open
  owner_uid, created_at, expires_at
  admins/{uid}
...
6. 管理員可更新 `questions_open`；關閉時，提問頁與安全規則都會拒絕新投稿。
7. 管理員可永久刪除留言板及其問題、授權、密鑰與首頁索引。
